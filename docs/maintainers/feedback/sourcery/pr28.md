...
- Consider extracting the default health URL (`http://localhost:5000/api/health`) into a single constant (or config default) rather than hardcoding it inside `_get_health_url` so it’s easier to change in one place later.

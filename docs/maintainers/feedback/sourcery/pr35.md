...
**Description**: The hard upper bounds on wall‑clock time (e.g. `< 0.1` seconds) are likely to be flaky on noisy or constrained CI. To make these tests more reliable, consider relaxing the thresholds (e.g. 250–500ms) so they act as coarse sanity checks, or rely on `@pytest.mark.performance` to run them separately/optionally (e.g. nightly) instead of on every push. You could also reduce timing noise by removing `print(...)` and using logging or pytest’s `--durations` for reporting instead.
...
**Description**: Here it’s used as a verb, so “Always back up the database before running migrations in production.” would be correct.
...
- In the performance tests you’re using `time.time()` and `print` with hard-coded thresholds (e.g., `< 0.1`); consider using `time.perf_counter()` and avoiding printing in tests, and perhaps relaxing or parametrizing the thresholds to reduce flakiness across different environments.
...
- The `test_config_set_and_get` test now monkeypatches attributes on a specific `Config()` singleton instance; this is a bit brittle—exposing a small helper or factory for injecting a config path, or monkeypatching at the class level via a dedicated test hook, would make the config behavior easier to test and maintain.
...
|---------|----------|--------|--------|--------|-------|
| #1 | 🟠 HIGH | 🟠 HIGH | 🟢 LOW | ✅ Fixed | Environment variable loading bug risk - Fixed in PR #35 (pr35-batch-high-low-01) |
| #2 | 🟠 HIGH | 🟠 HIGH | 🟢 LOW | ✅ Fixed | Database migration bug risk - Fixed in PR #35 (pr35-batch-high-low-01) |
| #3 | 🟡 MEDIUM | 🟡 MEDIUM | 🟢 LOW | Test assertion weakness - doesn't verify actual behavior. Should strengthen assertion. |
| #4 | 🟡 MEDIUM | 🟡 MEDIUM | 🟢 LOW | Test assertion weakness - may allow false positives. Consider more specific assertion. |
| #5 | 🟡 MEDIUM | 🟡 MEDIUM | 🟢 LOW | Performance test flakiness - tight thresholds may cause CI failures. Consider relaxing or using perf_counter. |
| #6 | 🟡 MEDIUM | 🟡 MEDIUM | 🟢 LOW | Test assertion weakness - only checks type, not value. Should assert empty list explicitly. |
| #7 | 🟢 LOW | 🟢 LOW | 🟢 LOW | Documentation typo - "backup" should be "back up". Quick fix. |
| #8 | 🟢 LOW | 🟢 LOW | 🟢 LOW | Documentation typo - "LOW/LOW" should be "LOW priority". Quick fix. |
| #9 | 🟡 MEDIUM | 🟡 MEDIUM | 🟡 MEDIUM | Loop in performance test - refactor to avoid loops. Code quality improvement. |
| #10 | 🟡 MEDIUM | 🟡 MEDIUM | 🟡 MEDIUM | Loop in performance test - refactor to avoid loops. Code quality improvement. |
| #11 | 🟡 MEDIUM | 🟡 MEDIUM | 🟡 MEDIUM | Loop in performance test - refactor to avoid loops. Code quality improvement. |
| #12 | 🟡 MEDIUM | 🟡 MEDIUM | 🟡 MEDIUM | Loop in performance test - refactor to avoid loops. Code quality improvement. |
| #13 | 🟡 MEDIUM | 🟡 MEDIUM | 🟡 MEDIUM | Loop in performance test - refactor to avoid loops. Code quality improvement. |
| #14 | 🟡 MEDIUM | 🟡 MEDIUM | 🟡 MEDIUM | Loop in performance test - refactor to avoid loops. Code quality improvement. |
| #15 | 🟡 MEDIUM | 🟡 MEDIUM | 🟡 MEDIUM | Loop in performance test - refactor to avoid loops. Code quality improvement. |

...
**Action:** Fix now - security issue, prevents information leakage
...
**🚨 issue (security):** Avoid returning raw exception messages in per-project error details to prevent leaking internal information.
...
**suggestion (testing):** Add a test for non‑JSON requests to exercise the `Content-Type` validation branch
...
**issue (code-quality):** Explicitly raise from a previous error [×3] ([`raise-from-previous-error`](https://docs.sourcery.ai/Reference/Default-Rules/suggestions/raise-from-previous-error/))
...
- The import endpoint currently treats a payload without a `projects` key or with a non-list `projects` value as a successful 201 with zero imports; consider validating the presence and type of `projects` and returning a 400 with a clear error message to align API behavior with the CLI’s expectations and make client errors easier to diagnose.
...
|---------|----------|--------|--------|-------|
| #1 | 🟡 MEDIUM | 🟡 MEDIUM | 🟢 LOW | Validate request body shape - prevents masking client errors |
| #2 | 🟠 HIGH | 🟠 HIGH | 🟢 LOW | Security: Don't expose exception messages to clients - fix now |
| #3 | 🟡 MEDIUM | 🟢 LOW | 🟢 LOW | Add test for non-JSON requests - improves test coverage |
| #4 | 🟢 LOW | 🟢 LOW | 🟡 MEDIUM | Avoid loop in test - code quality improvement |
| #5 | 🟢 LOW | 🟢 LOW | 🟡 MEDIUM | Avoid loop in test - code quality improvement |
| #6 | 🟢 LOW | 🟢 LOW | 🟡 MEDIUM | Avoid loop in test - code quality improvement |
| #7 | 🟢 LOW | 🟢 LOW | 🟡 MEDIUM | Avoid loop in test - code quality improvement |
| #8 | 🟢 LOW | 🟢 LOW | 🟢 LOW | Swap if expression - minor readability improvement |
| #9 | 🟢 LOW | 🟢 LOW | 🟢 LOW | Remove duplicate dict key - test cleanup |
| #10 | 🟡 MEDIUM | 🟢 LOW | 🟡 MEDIUM | Extract duplicate code - refactoring opportunity |
| #11 | 🟡 MEDIUM | 🟡 MEDIUM | 🟠 HIGH | Low code quality function - complex refactoring needed |
| #12 | 🟢 LOW | 🟢 LOW | 🟢 LOW | Raise from previous error - error context improvement |

...
**🚨 issue (security):** Avoid returning raw exception messages to clients to prevent leaking internal details and to keep error formats consistent.
...
**Description**: This acceptance criterion is listed but not backed by a concrete test case. Please add a short scenario (e.g., `GET /api/projects/9999` or `PATCH /api/projects/9999`) that specifies the expected 404 status and error response, so it’s clear how this should be validated.
...
| --------- | ----------- | ----------- | --------- | ---------------------------------------------------------- |
| #1        | 🔴 CRITICAL | 🔴 CRITICAL | 🟡 MEDIUM | Security: Exception details leaked to clients              |
| #2        | 🟠 HIGH     | 🟠 HIGH     | 🟡 MEDIUM | Bug: Null status causes confusing 409 instead of 400       |
| #3        | 🟡 MEDIUM   | 🟡 MEDIUM   | 🟢 LOW    | Missing test: Empty/no JSON body on POST                   |
| #4        | 🟢 LOW      | 🟢 LOW      | 🟢 LOW    | Docs typo: Missing article "a"                             |
| #5        | 🟢 LOW      | 🟡 MEDIUM   | 🟢 LOW    | Missing test scenario: 404 for non-existent projects       |
| #6        | 🟢 LOW      | 🟢 LOW      | 🟢 LOW    | Style: Merge nested if (classification validation - POST)  |
| #7        | 🟢 LOW      | 🟢 LOW      | 🟢 LOW    | Style: Merge nested if (status validation - POST)          |
| #8        | 🟢 LOW      | 🟢 LOW      | 🟢 LOW    | Style: Merge nested if (classification validation - PATCH) |
| #9        | 🟢 LOW      | 🟢 LOW      | 🟢 LOW    | Style: Merge nested if (status validation - PATCH)         |
| #10       | 🟢 LOW      | 🟢 LOW      | 🟢 LOW    | Style: Avoid loops in tests (classification)               |
| #11       | 🟢 LOW      | 🟢 LOW      | 🟢 LOW    | Style: Avoid loops in tests (status)                       |
| #12       | 🟢 LOW      | 🟢 LOW      | 🟢 LOW    | Style: Use walrus operator in create_project               |
| #13       | 🟢 LOW      | 🟢 LOW      | 🟢 LOW    | Style: Use walrus operator in update_project               |
| #14       | 🟡 MEDIUM   | 🟢 LOW      | 🟢 LOW    | Bare except in CLI create command                          |
| #15       | 🟢 LOW      | 🟢 LOW      | 🟢 LOW    | Style: Remove unnecessary .keys() call                     |
| Overall-1 | 🟡 MEDIUM   | 🟠 HIGH     | 🟠 HIGH   | Validation logic duplicated between POST/PATCH             |
| Overall-2 | 🟡 MEDIUM   | 🟡 MEDIUM   | 🟡 MEDIUM | Exception handling duplicated (related to #1)              |
| Overall-3 | 🟡 MEDIUM   | 🟡 MEDIUM   | 🟡 MEDIUM | CLI command logic duplication                              |

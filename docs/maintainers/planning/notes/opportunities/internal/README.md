# Internal Opportunities

**Purpose:** Export learnings from work-prod to other projects  
**Primary Target:** ~/Projects/dev-infra template  
**Status:** ✅ Active  
**Last Updated:** 2025-12-03

---

## 📋 Quick Links

### Phase Learnings
- **[Phase 1 Learnings](phase-1-learnings.md)** - List & Get Projects (Backend API + CLI)

### Template Improvements
- **[Dev-Infra Improvements](dev-infra-improvements.md)** - Actionable checklist for template updates

---

## 🎯 Purpose

This directory captures knowledge gained from work-prod implementation to:
1. **Improve dev-infra template** with proven patterns
2. **Share learnings** with other projects
3. **Document what works** for future reference
4. **Avoid repeating mistakes** in new projects

---

## 📝 Documentation Types

### Phase Learnings
Comprehensive reviews after each major phase:
- What worked well (to template)
- What needs improvement (to fix in template)
- Unexpected discoveries (to document)
- Time-saving patterns (to automate)

### Template Improvements
Actionable checklists for updating dev-infra:
- Specific file changes needed
- New patterns to add
- Documentation to enhance
- Automation opportunities

---

## 🔄 Review Cycle

**After Each Phase:** Capture learnings while fresh  
**Every 2-3 Phases:** Consolidate improvements  
**At MVP Complete:** Comprehensive template update

---

## ✅ Completion Tracking

| Phase | Learnings Doc | Status | Applied to dev-infra |
|-------|--------------|--------|---------------------|
| Phase 0 | (Included in Phase 1) | ✅ Complete | 🟡 Pending |
| Phase 1 | phase-1-learnings.md | ✅ Complete | 🟡 Pending |
| Phase 2+ | TBD | 🔴 Not Started | - |

---

**Last Updated:** 2025-12-03  
**Status:** ✅ Active  
**Next:** Apply Phase 1 learnings to dev-infra template

